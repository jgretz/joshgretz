...
export const createTaskRunner = async (setupCallback: SetupCallback): Promise<TaskRunnerContext> => {
  const config = getConfig();
  let retryCount = 0;
  let isShuttingDown = false;

  const attemptConnection = async (): Promise<PgBoss> => {
  ...
  const runWithRecovery = async (): Promise<TaskRunnerContext> => {
    while (true) {
      try {
        ...
        const shutdown = async () => {
          ...
          // Don't stop Docker postgres on normal shutdown for fast restart
        };
...
export const runWithAutoRecovery = async (setupCallback: SetupCallback): Promise<never> => {
  const config = getConfig();
