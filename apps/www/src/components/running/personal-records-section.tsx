...
const PRCard = ({pr}: {pr: PersonalRecord}) => (
  <div className="rounded-xl border border-warm-200 bg-white/60 px-4 py-6 text-center">
    ...
    <div className="mt-2 font-serif text-3xl text-warm-700">{formatTime(pr.time_seconds)}</div>
    {pr.pace_seconds && (
      <div className="mt-2 font-sans text-sm text-warm-400">{formatPace(pr.pace_seconds)}</div>
    )}
    {pr.race_name && (
    ...
    {(pr.race_date || pr.created_at) && (
      <div className="mt-2 font-sans text-sm text-warm-400">
        {formatDate((pr.race_date ?? pr.created_at) as string)}
      </div>
    )}
  </div>
);
